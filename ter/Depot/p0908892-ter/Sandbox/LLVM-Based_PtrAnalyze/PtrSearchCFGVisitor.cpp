...
class ConstPtrSearchCFGVisitor : public CFGRecStmtVisitor<ConstPtrSearchCFGVisitor> {
	public:
	// Visitor Mode
...
void ConstPtrSearchCFGVisitor::VisitArraySubscriptExpr(ArraySubscriptExpr *ASE)
{
	...
	switch (m_mode) {
		...
		case VM_Analyzing:
			...
			if ((DRE = dyn_cast<DeclRefExpr>(LHS)) && (VD = getVarDeclFromDeclRefExpr<PointerType>(DRE))) {
...
void ConstPtrSearchCFGVisitor::VisitDeclStmt(DeclStmt *DS) {
	...
	if ((VD = dyn_cast<VarDecl>(DS->getSingleDecl())) && isa<PointerType>(getTypePtrFromVarDecl(VD))) {
		...
		switch (m_mode) {
			...
			case VM_Gathering:
				...
				if ((E = VD->getInit()) && (UOP = dyn_cast<UnaryOperator>(E)) && (VD_INIT = getVarDeclFromAddrOfOperator(UOP)))
					m_pinfosMap[VD_INIT] = new PointerInfo(VD_INIT);
				break;

			// Analyzing mode
			case VM_Analyzing:
				if ((E = VD->getInit()) && (DRE = dyn_cast<DeclRefExpr>(E->IgnoreParenCasts())) && (VD_INIT = getVarDeclFromDeclRefExpr<PointerType>(DRE))) {
					m_aliasingManager.alias(VD, VD_INIT, true);
				}
				else if ((UOP = dyn_cast<UnaryOperator>(VD->getInit())) && (VD_INIT = getVarDeclFromAddrOfOperator(UOP)) && !isa<PointerType>(getTypePtrFromVarDecl(VD_INIT))) {
					m_aliasingManager.alias(VD, VD_INIT, true);
				}
				break;
