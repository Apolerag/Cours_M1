...
//===----------------------------------------------------------------------===//
class CFGBuilderRecursiveASTVisitor : public RecursiveASTVisitor<CFGBuilderRecursiveASTVisitor> {
	private:
...
bool CFGBuilderRecursiveASTVisitor::VisitFunctionDecl(FunctionDecl *f)  {
	if (f->hasBody()) {
		// Output
		outs() << "########################################\n";
		outs() << "Function with body found: " << f->getNameInfo().getAsString()<< "\n";
...
//===----------------------------------------------------------------------===//
class CFGBuilderASTConsumer : public ASTConsumer {
	private:
	CFGBuilderRecursiveASTVisitor &m_visitor; // AST Visitor to use

	public:
	// Constructor
	CFGBuilderASTConsumer(CFGBuilderRecursiveASTVisitor &visitor) : m_visitor(visitor) { }
