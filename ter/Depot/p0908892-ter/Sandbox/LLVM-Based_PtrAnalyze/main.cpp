...
//===----------------------------------------------------------------------===//
//
// First implem attempt for the analysis.
//
//===----------------------------------------------------------------------===//
#include <sys/types.h>
#include <sys/stat.h>
...
//===----------------------------------------------------------------------===//
#include "Utils.cpp"
#include "OldPointerInfo.cpp"
#include "AliasingManager.cpp"
#include "PtrSearchCFGVisitor.cpp"
...
int main(int argc, char *argv[]) {
	...
	if (optind != (argc - 1)) {
		errs() << "Usage: "PROGRAM_NAME" [OPTION] FILE\n";
		return 1;
	}
	...
	false,
	false);
	headerSearchOptions.AddPath("/usr/lib/llvm-3.3/include",
	clang::frontend::Angled,
	false,
	false);
